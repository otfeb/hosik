	...
	<%}
		...
		if(b){
			...
			<div style="margin: 10px 30px;">
				...
				<button type="button" class="btn btn-danger btn-xs" onclick="location.href='idcheck.jsp'">다시입력</button>
			</div>
		<%}
		else{
			//존재하지 않는경우
			...
			<div style="margin: 10px 30px;">
				...
				<img alt="" src="../image/b3.png" width="60" align="left">
				<button type="button" class="btn btn-info btn-xs" onclick="saveId('<%=id%>')">적용하기</button>
				<button type="button" class="btn btn-danger btn-xs" onclick="location.href='idcheck.jsp'">다시입력</button>
			</div>
		<%}
	}
...
<script type="text/javascript">
	function saveId(id) {
		...
		//부모창: opener / 현재창의 id를 부모창의 id에 나타내준다
		opener.mid.value=id;
		//현재창 닫기
		window.close();
	}
